...
export const prizeService = {
  ...
  get: (id: string) => api.get<Prize>(`/prizes/${id}`),
}
...
export const redemptionService = {
  ...
  list: (params?: { skip?: number; limit?: number; status?: string }) =>
    api.get<PaginatedResponse<PrizeRedemption>>("/prize-redemptions", {
      params,
    }),

  get: (id: string) => api.get<PrizeRedemption>(`/prize-redemptions/${id}`),
...
export const addressService = {
  ...
  create: (
    data: Omit<ShippingAddress, "id" | "user_id" | "created_at" | "updated_at">,
  ) => api.post<ShippingAddress>("/shipping-addresses", data),

  get: (id: string) => api.get<ShippingAddress>(`/shipping-addresses/${id}`),

  update: (
    id: string,
    data: Partial<
      Omit<ShippingAddress, "id" | "user_id" | "created_at" | "updated_at">
    >,
  ) => api.put<ShippingAddress>(`/shipping-addresses/${id}`, data),

  delete: (id: string) => api.delete(`/shipping-addresses/${id}`),
...
export const coinLogService = {
  ...
  get: (id: string) => api.get<CoinLog>(`/coin-logs/${id}`),
}
